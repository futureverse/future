 ...
 * `value()` on lists and and `listenv`:s gained optional argument
   `reduce`, which specifies a function for reducing all values,
   e.g. ``values(fs, reduce = `+`)``. Attribute `init` can be used to
   control the initial value, e.e. ``reduce = structure(`+`, init =
   42)``.
